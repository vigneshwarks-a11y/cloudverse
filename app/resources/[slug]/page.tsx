    ...
    <>
      <article className="pt-[140px] pb-12 lg:pt-[160px] lg:pb-16">
        <div className="cv-container max-w-3xl">
          <Link href="/resources" className="inline-flex items-center gap-1.5 text-sm text-white/65 hover:text-white mb-6">
            <ArrowLeft size={14} /> Resources
          </Link>
          <div className="flex items-center gap-3 text-[12px] text-white/55 mb-5">
          ...
          <h1 className="cv-h1 text-white">{r.title}</h1>
          <p className="cv-body-lg mt-6 text-white/75">{r.excerpt}</p>
        </div>
      </article>
          ...
          <div className="prose-cv">
            {r.body.split("\n\n").map((block, i) => {
              if (block.startsWith("**") && block.endsWith("**")) {
                return <h2 key={i} className="font-display font-bold text-white text-2xl mt-10 mb-4">{block.slice(2, -2)}</h2>;
              }
              const parts = block.split(/(\*\*[^*]+\*\*)/);
              return (
                <p key={i} className="text-white/80 text-[17px] leading-relaxed mt-5">
                  {parts.map((p, j) =>
