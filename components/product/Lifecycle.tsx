    ...
    <section className="cv-section bg-cv-surface2">
      <div className="cv-container">
        <div className="max-w-3xl mb-12">
          ...
          <h2 className="cv-h2 text-white">Inform. Optimize. Operate.</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-0 border-t border-b border-white/10">
          {stages.map((s, i) => (
            <div
              key={s.title}
              className={`p-6 sm:p-8 ${i < 2 ? "md:border-r border-white/10" : ""} ${i > 0 ? "border-t md:border-t-0 border-white/10" : ""}`}
            >
