...
const TENANTS: { name: string; policy: string; gpu: number; cap: string; color: string }[] = [
  { name: "Acme AI", policy: "Strict-Prod", gpu: 42, cap: "$4.2k/mo", color: "#1664C0" },
  { name: "Beta Labs", policy: "Flexible", gpu: 31, cap: "$3.1k/mo", color: "#6954D4" },
  { name: "Core Eng", policy: "Residency-EU", gpu: 27, cap: "$2.7k/mo", color: "#0E9E7A" },
];
...
function MockTenantVisual() {
  return (
    <div className="mt-6 h-[430px]">
      <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-cv-line bg-white p-4 shadow-[0_10px_28px_-14px_rgba(16,24,40,0.10)] dark:shadow-[0_20px_50px_-20px_rgba(0,0,0,0.5)] dark:border-white/10 dark:bg-black" style={EDGE_FADE}>
        <CardLightEdge />
        <div className="px-1 pb-3 text-base font-semibold text-cv-ink">Workspaces</div>

        {/* column header */}
        <div className="grid grid-cols-[1.2fr_1fr_1.1fr_auto] gap-3 border-b border-cv-line/60 px-3 pb-2 text-[10px] font-semibold uppercase tracking-wider text-cv-muted dark:border-white/5">
          <span>Workspace</span>
          <span>Policy</span>
          <span>GPU alloc.</span>
        ...
        <div className="flex flex-1 flex-col justify-between gap-2.5 pt-2.5">
          {TENANTS.map((t) => (
            <div key={t.name} className="grid grid-cols-[1.2fr_1fr_1.1fr_auto] items-center gap-3 rounded-xl border border-cv-line/70 bg-cv-ink/[0.02] px-3 py-3 dark:border-white/[0.07] dark:bg-white/[0.02]">
              <span className="flex items-center gap-2 min-w-0">
                <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: t.color }} />
                <span className="truncate text-sm font-medium text-cv-ink">{t.name}</span>
              </span>
              ...
              <span className="flex items-center gap-2">
                ...
                <span className="font-mono text-[11px] text-cv-muted">{t.gpu}%</span>
              </span>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-lg border border-[#1664C0]/25 bg-[#1664C0]/[0.06] px-3 py-2 text-[11px] text-cv-ink/70">
            ...
            return (
              <div key={b.team}>
                <div className="mb-2 flex items-baseline justify-between">
                  <span className="flex items-center gap-2 text-sm font-medium text-cv-ink">
                    {b.team}
                    {b.flag && (
                      <span className="rounded-full bg-[#D97706]/15 px-2 py-0.5 text-[10px] font-semibold text-[#D97706]">Approaching cap</span>
                    )}
                  </span>
                  <span className="font-mono text-xs text-cv-muted">
                    <span className="text-cv-ink">${b.spent}k</span> / ${b.limit}k
                  </span>
                </div>
                <div className="h-2.5 w-full overflow-hidden rounded-full bg-cv-ink/[0.07] dark:bg-white/[0.07]">
                  <div className="h-full rounded-full" style={{ width: `${b.pct}%`, background: `linear-gradient(90deg, ${c}, ${c}CC)`, boxShadow: `0 0 10px ${c}66` }} />
                </div>
                <div className="mt-1 text-right font-mono text-[11px]" style={{ color: c }}>{b.pct}%</div>
              </div>
            );
          })}
        </div>

        <div className="mt-3 flex items-center gap-2 rounded-lg border border-cv-line/60 bg-cv-ink/[0.02] px-3 py-2 text-[11px] text-cv-ink/70 dark:border-white/10 dark:bg-white/[0.02]">
