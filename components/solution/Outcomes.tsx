      ...
      <div className="cv-container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {items.map((o) => {
            const Icon = o.icon;
            return (
              <div key={o.title} className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
                <div className="w-10 h-10 rounded-lg bg-cv-blue/15 text-cv-blue-light flex items-center justify-center mb-4">
                  <Icon size={20} />
                </div>
                <div className="font-display font-semibold text-white text-lg">{o.title}</div>
                <p className="text-white/65 text-sm mt-2 leading-relaxed">{o.body}</p>
                {o.metric && (
